...
if (!app.requestSingleInstanceLock()) {
  ...
  void app.whenReady().then(async () => {
    ...
    notesGitService.start().catch((error) => {
      logger.error('Failed to start notes git service', error as Error)
    })
    ...
    // Selection assistant is disabled (kept as no-op init for compatibility)
    initSelectionService()

    void runAsyncFunction(async () => {
      // Start API server only when explicitly enabled to avoid background
      // listeners during normal desktop-only usage.
      try {
