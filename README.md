...
- **会话与标签体验**：实现话题多选与批量管理（批量移动/删除/导出）、“全选并去重”、批量生成话题名、置顶优先排序与手动拖拽排序；完善 Tab 新建入口、Tab 聊天状态记忆、启动台助手/话题快捷选择与自动切换逻辑。
- **导入导出能力**：补齐小程序导出链路（WebView 脚本通道）、整合 ChatGPT/Gemini 导出到统一导入流程，新增 Gemini 导入器，并完善导出过程的加载态与完成提示。
- **模型与推理体验**：完善 Thinking Block 的“思考程度/耗时”展示与回放一致性（metadata），补充 `reasoning_effort`（含 `xhigh`）相关能力、徽标和测试。
- **Agent / Skills 能力**：新增并打磨输入栏 Skills 注入面板、OpenClaw dashboard 鉴权令牌持久化，以及 Agent 推理参数兼容性修复。
- **数据能力（笔记/备份）**：新增增量备份与默认清理策略；增强笔记（折叠留白、版本恢复、Git 集成）并修复 IPC 克隆相关稳定性问题。
- **平台兼容与工程质量**：修复 macOS 字体枚举与 CSS 覆盖问题，降低 ApiServer 状态刷屏；持续维护 workflows、lint/typecheck/test、快照与超时配置，保证分支长期可维护。

**阶段性里程碑（摘要）**
- 2025-12：完成多选与批处理、Tab/启动台交互重构、导入导出主链路落地。
- 2026-01～2026-02：补齐笔记与备份能力，完善 provider / OpenClaw / 设置项，持续修复工程构建与 CI 细节。
- 2026-03～2026-05：集中在 Skills/Agent 细化、推理参数兼容、渲染与稳定性回归修复。

<details>
  <summary>atoz03 贡献统计与口径（持续更新）</summary>

说明：
- 统计口径：`git log --all --author="atoz03" --no-merges`
- 截至时间：`2026-05-09`
- 时间范围：`2025-12-04` ～ `2026-05-09`
- 非合并提交总数：`273`
- 提交类型分布：`fix 106` / `feat 82` / `test 11` / `docs 11` / `chore 11` / `refactor 6`

可复现命令：

```bash
git log --all --author="atoz03" --no-merges --pretty=format:"%ad|%h|%s" --date=short
```

如需查看完整提交列表，建议直接按作者过滤查看历史（含上下文 diff）：
- https://github.com/CherryHQ/cherry-studio/commits?author=atoz03
