...
- 话题多选与批量管理：右键开启/退出多选，顶部操作条显示已选数；支持全选/清空、批量移动/删除、导出 ChatGPT 导入 JSON（按助手名+时间戳命名），移动时按目标助手话题时间排序并自动处理激活话题补位。
- 多选效率增强：新增“全选并去重”（按标题规范化去重）、批量生成话题名（并发上限 5、空消息跳过、逐条提示），移动话题去重与右键多选交互修复。
- 排序与整理：多选模式提供排序开关（按更新时间/创建时间倒序、手动拖拽并记忆顺序），话题列表置顶优先排序。
- 助手间一键迁移全部话题：新增 moveAllTopics 流程（Redux/数据库/i18n/确认弹窗），完善提示与相关测试。
- 启动台与导航优化：启动台加入助手/话题快捷选择并持久化，选择同步与自动话题切换行为修复。
